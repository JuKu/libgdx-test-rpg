...
public class SpriteBatcherUtils {
    ...
    public static void drawRect (SpriteBatch batch, Rectangle hitbox, float thickness, Color color) {
        //draw line
        /*SpriteBatcherUtils.drawLine(batch, hitbox.getX(), hitbox.getY(), hitbox.getX() + hitbox.getWidth(), hitbox.getY(), color);

        //draw line
        SpriteBatcherUtils.drawLine(batch, hitbox.getX(), hitbox.getY(), hitbox.getX(), hitbox.getY() + hitbox.getHeight(), color);

        //draw line
        SpriteBatcherUtils.drawLine(batch, hitbox.getX(), hitbox.getY() + hitbox.getHeight(), hitbox.getX() + hitbox.getWidth(), hitbox.getY() + hitbox.getHeight(), color);

        //draw line
        SpriteBatcherUtils.drawLine(batch, hitbox.getX() + hitbox.getWidth(), hitbox.getY(), hitbox.getX() + hitbox.getWidth(), hitbox.getY() + hitbox.getHeight(), color);*/
    ...
    /*public static void drawLine (SpriteBatch batch, float x1, float y1, float x2, float y2, Color color) {
        ...
        affine2.setToTranslation(x1, y1);
        affine2.setToRotation(angle);
        ...
        System.out.println("draw line, x: " + x1 + ", y: " + y1 + ", width: " + dx + ", height: " + dy + ".");

        batch.draw(textureRegion, dx * thickness, dy * thickness, affine2);
        //batch.draw(pixelTexture, Math.round(x1), Math.round(y1), dist, 0, 0, rad);
    ...
    protected static void initTextureIfAbsent () {
        if (pixelTexture == null) {
            ...
            /*if (!pixelTexture.isManaged()) {
                throw new IllegalStateException("Could found utils/onepixel.png in resource directory.");
            }*/
